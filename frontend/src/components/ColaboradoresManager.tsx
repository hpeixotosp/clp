...
import { useColaboradores } from "@/hooks/useApi";
...
function ColaboradoresManager({ onColaboradoresChange }: ColaboradoresManagerProps) {
  ...
  const carregarColaboradores = useCallback(async () => {
    try {
      ...
      setListaCompleta(colaboradores.join('\n'));
      onColaboradoresChange?.(colaboradores);
    } catch (err) {
      console.error('Erro ao carregar colaboradores:', err);
    }
  }, [loadColaboradores, onColaboradoresChange, colaboradores]);

  useEffect(() => {
    carregarColaboradores();
  }, [carregarColaboradores]);

  // Sincronizar lista completa quando colaboradores mudarem
  useEffect(() => {
    setListaCompleta(colaboradores.join('\n'));
  }, [colaboradores]);
  ...
  const adicionarColaborador = async () => {
    ...
    try {
      ...
      await addColaborador(novoColaborador.trim());
      setListaCompleta(colaboradores.join('\n'));
      setNovoColaborador("");
  ...
  const salvarListaCompleta = async () => {
    try {
      ...
      await replaceAll(novosColaboradores);
      setListaCompleta(colaboradores.join('\n'));
      setDialogAberto(false);
      setSuccess("Lista de colaboradores atualizada com sucesso!");
      onColaboradoresChange?.(colaboradores);
